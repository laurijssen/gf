...
def follow(username):
    ...
    if user is None:
        flash('Invalid user {}'.format(username))
        return redirect(url_for('.index'))

    if current_user.is_following(user):
        flash('You are already following the user {}'.format(username))
...
def followed_by(username):
    ...
    pagination = user.followed.paginate(
        page, per_page=int(current_app.config['FOLLOWERS_PER_PAGE']),
        error_out=False)
...
def post(id):
    ...
    page = request.args.get('page', 1, type=int)
    if page == -1:
        page = (post.comments.count() -1) // int(current_app.config['COMMENTS_PER_PAGE'])

    pagination = post.comments.order_by(Comment.timestamp.asc()).paginate(page, per_page=int(current_app.config['COMMENTS_PER_PAGE']), error_out=False)
